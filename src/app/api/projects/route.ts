...
export async function GET() {
  ...
  const projects = db.prepare(`
    SELECT
      ...
      COUNT(CASE WHEN sub.title NOT LIKE '[N2N%' AND sub.title NOT LIKE '[AUDIT]%' AND sub.status != 'cancelled' THEN 1 END) as task_count,
      SUM(CASE WHEN sub.status = 'done' AND sub.title NOT LIKE '[N2N%' AND sub.title NOT LIKE '[AUDIT]%' THEN 1 ELSE 0 END) as completed_tasks,
      SUM(CASE WHEN sub.status = 'failed' AND sub.title NOT LIKE '[N2N%' AND sub.title NOT LIKE '[AUDIT]%' AND sub.status != 'cancelled' THEN 1 ELSE 0 END) as failed_tasks,
